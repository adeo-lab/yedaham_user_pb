	...
	const _ = {
		...
		gnbMenu: {
			...
			cleanup: function() {
				...
				if (naviMenuBtn && _.toggleMenuListener) { // 새로운 리스너 이름 사용
					naviMenuBtn.removeEventListener('click', _.toggleMenuListener);
					_.toggleMenuListener = null;
				}
			},
			pc: function() {
				...
				const dep3Links = document.querySelectorAll('.lnb-item.dep3 > .lnb-text');
				dep3Links.forEach(function(dep3Link) {
					dep3Link.addEventListener('click', function(event) {
						event.preventDefault(); 
						this.closest('.lnb-item.dep3').classList.toggle('on');
					});
				});
			},
			mo: function() {
				...
				// naviMenuCloseBtn 변수 제거
				if (!(naviMenuBtn && navilMenu)) return;

				// 하나의 함수로 열고 닫는 기능 통합
				_.toggleMenuListener = function() {
					// 버튼에 'active' 클래스가 있는지 확인하여 상태를 토글
					const isOpen = naviMenuBtn.classList.contains('active');
				...
				// 하나의 버튼에 토글 리스너만 추가
				naviMenuBtn.addEventListener('click', _.toggleMenuListener);
				...
				dep3Links.forEach(function(dep3Link) {
					dep3Link.addEventListener('click', function(event) {
						event.preventDefault(); 
						const parentItem = this.closest('.lnb-item.dep3');
						if (parentItem) {
